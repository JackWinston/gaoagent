...
2. 不调用工具时：
   ...
   - thought 格式的Json：{{"type":"thought","content":"你要输出的内容"}}
   - final 格式的Json：{{"type":"final","content":"你要输出的内容"}} 
3. 禁止输出 Markdown 代码块、额外解释文字。
5. 收到tool_calls result后，必须先输出 thought格式的Json，再决定是否继续调用工具或给 final格式的Json。禁止连续使用工具。
