...
class TestCoreHandlers(unittest.TestCase):
    def test_init_calls_core_init(self) -> None:
        ...
        with patch("gaoagent.core.CoreHandlers.CoreInit") as mock_core_init:
            handlers.init()
    ...
    def test_config_calls_core_config_default(self) -> None:
        handlers = CoreHandlers()
        with patch(
            "gaoagent.core.CoreHandlers.CoreConfigDefault"
    ...
    def test_chat_forwards_all_args(self) -> None:
        handlers = CoreHandlers()
        with patch("gaoagent.core.CoreHandlers.ChatRunner") as mock_chat_runner:
            handlers.chat(
                new=True,
                prompt="你好",
                api="openai",
    ...
    def test_task_forwards_required_and_optional_args(self) -> None:
        handlers = CoreHandlers()
        with patch("gaoagent.core.CoreHandlers.TaskRunner") as mock_task_runner:
